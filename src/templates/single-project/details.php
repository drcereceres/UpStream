...
if ( $projectDateStartIsNotEmpty || $projectDateEndIsNotEmpty ) {
    if ( ! $projectDateEndIsNotEmpty ) {
        $projectTimeframe = '<i class="text-muted">' . __( 'Start Date',
                'upstream' ) . ': </i>' . upstream_format_date( $project->dateStart );
    } elseif ( ! $projectDateStartIsNotEmpty ) {
        $projectTimeframe = '<i class="text-muted">' . __( 'End Date',
                'upstream' ) . ': </i>' . upstream_format_date( $project->dateEnd );
    } else {
            ...
            <div class="row">
                ...
                <?php if ( $project->owner_id > 0 ): ?>
                    <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
                        ...
                        <span><?php echo $project->owner_id > 0 ? upstream_user_avatar( $project->owner_id ) : '<i class="text-muted">(' . __( 'none',
                                    'upstream' ) . ')</i>'; ?></span>
                    </div>
                <?php endif; ?>
