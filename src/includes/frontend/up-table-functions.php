        ...
        'milestone'   => [
            ...
            'renderCallback' => function ( $columnName, $columnValue, $column, $row, $rowType, $projectId ) use (
                ...
                $milestone = ! isset( $milestones[ $columnValue ] )
                    ? '<span title="' . __( "This Milestone doesn't exist anymore.",
                        'upstream' ) . '">' . $columnValue . ' <small><i class="fa fa-ban"></i></small></span>'
            ...
            'renderCallback' => function ( $columnName, $columnValue, $column, $row, $rowType, $projectId ) use (
                ...
                if ( strlen( $columnValue ) > 0 ) {
                    ...
                    if ( isset( $milestones[ $columnValue ] ) ) {
                        $columnValue = sprintf( '<span class="label up-o-label" style="background-color: %s;">%s</span>',
                            $milestones[ $columnValue ]['color'], $milestones[ $columnValue ]['title'] );
                    } else {
                        $columnValue = sprintf(
                            '<span class="label up-o-label" title="%s" style="background-color: %s;">%s <i class="fa fa-ban"></i></span>',
            ...
            'renderCallback' => function ( $columnName, $columnValue, $column, $row, $rowType, $projectId ) use (
                ...
                if ( strlen( $columnValue ) > 0 ) {
                    if ( isset( $severities[ $columnValue ] ) ) {
                        $columnValue = sprintf( '<span class="label up-o-label" style="background-color: %s;">%s</span>',
                            $severities[ $columnValue ]['color'], $severities[ $columnValue ]['name'] );
                    } else {
                        $columnValue = sprintf(
                            '<span class="label up-o-label" title="%s" style="background-color: %s;">%s <i class="fa fa-ban"></i></span>',
...
function renderTableColumnValue( $columnName, $columnValue, $column, $row, $rowType, $projectId ) {
    ...
    if ( $columnType === 'user' ) {
        ...
        if ( isset( $column['renderCallback'] ) && is_callable( $column['renderCallback'] ) ) {
            $html = call_user_func( $column['renderCallback'], $columnName, $columnValue, $column, $row, $rowType,
                $projectId );
        }
                ...
                <div>
                    <?php foreach ( $hiddenColumnsSchema as $columnName => $column ):
                        ...
                        <div class="form-group" data-column="<?php echo $columnName; ?>">
                            ...
                            <?php renderTableColumnValue( $columnName, $columnValue, $column, $row, $rowType,
                                $projectId ); ?>
                        </div>
                    <?php endforeach; ?>
                </div>
...
function renderTable( $tableAttrs = [], $columnsSchema = [], $data = [], $itemType = '', $projectId = 0 ) {
    $tableAttrs['class'] = array_filter( isset( $tableAttrs['class'] ) ? ( ! is_array( $tableAttrs['class'] ) ? explode( ' ',
        $tableAttrs['class'] ) : (array) $tableAttrs['class'] ) : [] );
