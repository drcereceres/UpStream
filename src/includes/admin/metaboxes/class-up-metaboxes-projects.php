    ...
    class UpStream_Metaboxes_Projects {
            ...
            if ( ( ! $areMilestonesDisabled && $areMilestonesDisabledAtAll ) || ! $areTasksDisabled || ! $areBugsDisabled ) {
                ...
                if ( ! $areMilestonesDisabled && ! $areMilestonesDisabledAtAll ) {
                    array_push( $columnsList, $metabox->add_field( [
                        'name'  => '<span>' . upstream_count_total( 'milestones',
                                upstream_post_id() ) . '</span> ' . upstream_milestone_label_plural(),
                ...
                if ( ! upstream_disable_tasks() ) {
                    if ( ! $areTasksDisabled ) {
                        $grid2 = $metabox->add_field( [
                            'name'  => '<span>' . upstream_count_total( 'tasks',
                                    upstream_post_id() ) . '</span> ' . upstream_task_label_plural(),
                            'desc'  => '',
                ...
                if ( ! $areBugsDisabled ) {
                    $grid3 = $metabox->add_field( [
                        'name'  => '<span>' . upstream_count_total( 'bugs',
                                upstream_post_id() ) . '</span> ' . upstream_bug_label_plural(),
                        'desc'  => '',
                        'id'    => $this->prefix . 'bugs',
                        'type'  => 'title',
                        'after' => 'upstream_output_overview_counts',
            ...
            if ( ! $areMilestonesDisabled ) {
                ...
                if ( $allowComments ) {
                    ...
                    <div class="up-c-tabs-header nav-tab-wrapper nav-tab-wrapper">
                      <a href="#" class="nav-tab nav-tab-active up-o-tab up-o-tab-data" role="tab" data-target=".up-c-tab-content-data">' . __( 'Data',
                            'upstream' ) . '</a>
        ...
        private function getMilestonesFiltersHtml() {
            ...
            <div class="up-c-filters">
                ...
                <div class="up-c-filter">
                    <label for="<?php echo $prefix . 'assignee'; ?>" class="up-s-mb-2"><?php _e( 'Assignee',
                            'upstream' ); ?></label>
        ...
        private function getTasksFiltersHtml() {
            ...
            <div class="up-c-filters">
                ...
                <div class="up-c-filter">
                    <label for="<?php echo $prefix . 'status'; ?>" class="up-s-mb-2"><?php _e( 'Status',
                            'upstream' ); ?></label>
        ...
        private function getBugsFiltersHtml() {
            ...
            <div class="up-c-filters">
                ...
                <div class="up-c-filter">
                    <label for="<?php echo $prefix . 'severity'; ?>" class="up-s-mb-2"><?php _e( 'Severities',
                            'upstream' ); ?></label>
        ...
        private function getFilesFiltersHtml() {
            ...
            <div class="up-c-filters">
                ...
                <div class="up-c-filter">
                    <label for="<?php echo $prefix . 'uploaded_by'; ?>" class="up-s-mb-2"><?php _e( 'Uploaded by',
                            'upstream' ); ?></label>
            ...
            if ( ! $areTasksDisabled ) {
                ...
                if ( ! upstream_are_milestones_disabled() && ! upstream_disable_milestones() ) {
                    $fields[41] = [
                        ...
                        'id'               => 'milestone',
                        'desc'             =>
                            __( 'Selecting a milestone will count this task\'s progress toward that milestone as well as overall project progress.',
                                'upstream' ),
                        'type'             => 'select',
        ...
        public function details() {
            $metabox = new_cmb2_box( [
                ...
                'title'        => '<span class="dashicons dashicons-admin-generic"></span> ' . sprintf( __( "%s Details",
                        'upstream' ), $this->project_label ),
                'object_types' => [ $this->type ],
                'context'      => 'side',
                'priority'     => 'high',
            ] );
        ...
        public static function fetchAllItemsComments() {
            ...
            try {
                ...
                foreach ( $itemsTypes as $itemType ) {
                    ...
                    $rowset           = array_filter( (array) get_post_meta( $project_id,
                        '_upstream_project_' . $itemType, true ) );
                    if ( count( $rowset ) > 0 ) {
        ...
        public static function renderSelect2Field( $field, $value, $object_id, $objectType, $fieldType ) {
            ...
            <select
                ...
                <?php foreach ( $options as $optionValue => $optionTitle ): ?>
                    <option value="<?php echo esc_attr( $optionValue ); ?>"<?php echo in_array( $optionValue,
                        $value ) ? ' selected' : ''; ?>><?php echo esc_html( $optionTitle ); ?></option>
                <?php endforeach; ?>
            </select>
            <?php
        }
