        ...
        jQuery(document).ready(function ($) {
            ...
            function checkValidation (evt) {
                ...
                if ($first_error_row) {
                    ...
                    $('#major-publishing-actions').append($('<div class="notice notice-error"><?php _e( 'Missing some required fields',
                        'upstream' ) ?></div>').hide().fadeIn(500));
...
function upstream_output_overview_counts( $field_args, $field ) {
    ...
    <div class="counts <?php echo esc_attr( $itemType ); ?>">
        ...
        <h4>
            <span
                class="count open<?php echo esc_attr( $countMine > 0 ? ' mine' : '' ); ?>"><?php echo (int) $countMine ?></span> <?php _e( 'Mine',
                'upstream' ); ?>
        </h4>
    </div>
    <?php
...
function upstream_activity_buttons( $field_args, $field ) {
    ...
    $edit_buttons .= '<a class="button button-small' . esc_attr( $_10 ) . '" href="' . esc_url( add_query_arg( 'activity_items',
            '10' ) ) . '" >' . __( 'Last 10', 'upstream' ) . '</a> ';
    $edit_buttons .= '<a class="button button-small' . esc_attr( $_20 ) . '" href="' . esc_url( add_query_arg( 'activity_items',
            '20' ) ) . '" >' . __( 'Last 20', 'upstream' ) . '</a> ';
    $edit_buttons .= '<a class="button button-small' . esc_attr( $_all ) . '" href="' . esc_url( add_query_arg( 'activity_items',
            'all' ) ) . '" >' . __( 'View All', 'upstream' ) . '</a> ';
    $edit_buttons .= '</div>';
                ...
                <?php if ( ! $isApproved && $currentUserCapabilities->can_moderate ): ?>
                    <div class="u-text-center">
                        <i class="fa fa-eye-slash u-color-gray"
                           title="<?php _e( "This comment and its replies are not visible by regular users.",
                               'upstream' ); ?>" style="margin-top: 2px;"></i>
                    </div>
                <?php endif; ?>
            </div>
...
function upstream_admin_get_all_project_users() {
    ...
    if ( $projectId > 0 ) {
        ...
        if ( $projectClientId > 0 ) {
            $projectClientUsersIds = array_filter( array_map( 'intval',
