<?php
// @todo : remove this file when the issue's closed
...
class UpStream_Metaboxes_Clients {
    ...
    public function __construct() {
        ...
        //add_action('wp_insert_post_data', array('UpStream_Metaboxes_Clients', 'kluster'), 99, 2);
        add_action('save_post', array('UpStream_Metaboxes_Clients', 'save_post'));
        add_action('wp_ajax_upstream_email_validation', array($this, 'validateEmail'));
    }
    ...
    public static function validateEmail()
    {
        ...
        if (!empty($_POST) && isset($_POST['subject'])) {
            $email = trim($_POST['subject']);

            // Check if the email is potentially valid.
            if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                // Check if this email address is already registered to someone else.
                $response['email'] = email_exists($email);
            } else {
                $response['err'] = __('Invalid email address.', 'upstream');
            }
        }
    ...
    public static function kluster($data, $post)
    {
        return null;
