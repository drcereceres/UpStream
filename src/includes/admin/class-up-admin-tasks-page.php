...
class Upstream_Task_List extends WP_List_Table {
    ...
    function get_views() {
        ...
        $views['all'] = "<a href='" . esc_url( $all_url ) . "' " . esc_attr( $all_class ) . " >" . __( 'All',
                'upstream' ) . "</a>(" . esc_html( $all_count ) . ")";
        ...
        $views['mine'] = "<a href='" . esc_url( $mine_url ) . "' " . esc_attr( $mine_class ) . " >" . __( 'Mine',
                'upstream' ) . "</a>(" . esc_html( $mine_count ) . ")";
                ...
                if ( ! empty( $projects ) ) { ?>
                    ...
                    <select name='project' id='project' class='postform'>
                        ...
                        <?php foreach ( $projects as $project_id => $title ) { ?>
                            <option
                                value="<?php echo $project_id ?>" <?php isset( $_GET['project'] ) ? selected( $_GET['project'],
                                $project_id ) : ''; ?>><?php echo esc_html( $title ) ?></option>
                        <?php } ?>
                    </select>
                ...
                if ( ! empty( $assigned_to ) ) { ?>
                    ...
                    <select name='assigned_to' id='assigned_to' class='postform'>
                        ...
                        <?php foreach ( $assigned_to as $user_id => $user ) { ?>
                            <option
                                value="<?php echo $user_id ?>" <?php isset( $_GET['assigned_to'] ) ? selected( $_GET['assigned_to'],
                                $user_id ) : ''; ?>><?php echo esc_html( $user ) ?></option>
                        <?php } ?>
                    </select>
                ...
                if ( ! empty( $status ) ) { ?>
                    ...
                    <select name='status' id='status' class='postform'>
                        ...
                        <?php foreach ( $status as $stati ) {
                            ...
                            ?>
                            <option
                                value="<?php echo $statusId; ?>" <?php isset( $_REQUEST['status'] ) ? selected( $_REQUEST['status'],
                                $statusId ) : ''; ?>><?php echo esc_html( $statusTitle ) ?></option>
                        <?php } ?>
                    </select>
    ...
    public function column_default( $item, $column_name ) {
        ...
        switch ( $column_name ) {
            ...
            case 'project':
                ...
                $output .= '<br>' . esc_html( upstream_project_progress( $item['project_id'] ) ) . '% ' . __( 'Complete',
                        'upstream' );
            ...
            case 'milestone':
                ...
                if ( ! upstream_are_milestones_disabled( $item['project_id'] ) ) {
                    if ( isset( $item['milestone'] ) && ! empty( $item['milestone'] ) ) {
                        ...
                        return $milestone ? esc_html( $milestoneTitle ) . '<br>' . esc_html( $progress ) . '% ' . __( 'Complete',
                                'upstream' ) : '';
                    }
                }
