...
class Comments {
            ...
            if ( $comment->parent_id > 0 ) {
                ...
                if ( is_numeric( $parentComment->comment_approved ) ) {
                    if ( (bool) $parentComment->comment_approved ) {
                        ...
                        $userCanModerateComments  = ! $userHasAdminCapabilities ? user_can( $user,
    ...
    private static function toggleCommentApprovalStatus( $comment_id, $isApproved ) {
        ...
        if (
            ...
            || ! isset( $_POST['project_id'] )
            || ! isset( $_POST['comment_id'] )
            || ! check_ajax_referer( 'upstream:project.' . ( $isApproved ? 'approve_comment' : 'unapprove_comment' ) . ':' . $_POST['comment_id'],
                'nonce', false )
        ) {
            throw new \Exception( __( 'Invalid request.', 'upstream' ) );
        }
    ...
    public static function fetchComments() {
        ...
        try {
            ...
            $itemsRowset = (array) get_post_meta( $project_id, '_upstream_project_' . $commentTargetItemType . 's',
                true );
    ...
    public static function defineNotificationRecipients( $recipients, $comment_id ) {
        ...
        if ( empty( $project ) ) {
            ...
            set_transient( 'upstream:comment_notification.project:' . $comment->project_id, $project,
                $transientExpiration );
        } else {
            if ( $comment->target !== 'project'
